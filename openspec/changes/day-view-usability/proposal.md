...
- **The day lane's numbers become exact worked · reportable rounded · balance.** The headline is exact, so it reconciles with the balance beside it; the rounded half-hour value rides along as a subordinate `~8.0h` — marked approximate so it does not invite arithmetic, with its purpose in the accessible name rather than in row space it would repeat on every lane. **BREAKING** for the `worktime-calculation` presentation requirement, which is restated around the rule that actually matters: every figure standing in an arithmetic relationship to another must share its basis. The reportable value stands in none.
